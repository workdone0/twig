...
class Column(Vertical):
    ...
    def find_next(self, query: str, direction: int = 1) -> bool:
        """
...
class ColumnNavigator(HorizontalScroll):
    ...
    def _expand_node(self, column_index: int, node_id: uuid.UUID, initial_select_index: int = 0) -> None:
    ...
    def find_next(self, query: str, direction: int = 1) -> bool:
        ...
        for idx in range(start_index, max_index + 1):
            ...
            if col:
                found = col.find_next(query, direction)
                if found:
                ...
                if parent_id:
                    ...
                    if found:
                        ...
                        for choice_idx in path:
                             ...
                             self._expand_node(current_col_idx, current_node_id, initial_select_index=choice_idx)
                        ...
                        self._expand_node(current_col_idx, current_node_id, initial_select_index=match_idx)
                            ...
                            for child in self.children:
                                if isinstance(child, Column) and child.index == final_col_idx:
                                    ...
                                    if c_node_id:
                                        ...
                                        # We simulate a Highlighted event or call _expand_node directly
                                        self.on_column_highlighted(Column.Highlighted(child.index, c_node_id))
                                    break
                        self.call_after_refresh(focus_final)

                        return True

        return focused_col.find_next(query, direction)
    ...
    def on_column_highlighted(self, message: Column.Highlighted) -> None:
        ...
        self._expand_node(message.column_index, message.node_id, initial_select_index=0)
